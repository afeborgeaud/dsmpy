            ...
            if ( spo==int(spo) ) mtmp = mtmp - 1
                ...
                do j=1,ndc+1
                    if ( iphase(j)==1 ) then
                        ...
                        jtmp = jdr+jsp(j)
                        mtmp = kdr+ksp(j)
                        call cala0( nlayer(j),omega,omegai,t(itmp),h1x(itmp), h1y(itmp),h1z(itmp),&
                                            h2L(itmp), h2N(itmp),h3ax(itmp),h3ay(itmp),h3az(itmp),&
                                            h4aL(itmp),h4aN(itmp),h5ax(itmp),h5ay(itmp),h5az(itmp),&
                                            h6aL(itmp),h6aN(itmp),h7x(itmp),h7y(itmp),h7z(itmp),&
                                            h8L(itmp), h8N(itmp),coef1(j),coef2(j),cwork(jtmp) )
                        call overlapa( nlayer(j),cwork(jtmp),a0(1,mtmp) )
                        call cala1( nlayer(j),h1x(itmp),h1y(itmp),h1z(itmp),&
                                            h2L(itmp),h2N(itmp),h3x(itmp), h3y(itmp), h3z(itmp),&
                                            h4L(itmp), h4N(itmp),h5x(itmp), h5y(itmp), h5z(itmp),&
                        ...
                        call cala2( nlayer(j),h1x(itmp), h1y(itmp),h1z(itmp),&
                                            h2L(itmp),h2N(itmp),coef1(j),coef2(j),cwork(jtmp) )
                 ...
                 !   c --- renewing  mdr
                    ...
                    !        c computing the excitation vector
                            ...
                            if ( l==0 ) then
                                ...
                                if( mod(l,100)==0) call calcutd(nzone,nlayer,tmpc,ratc,nn,iphase,spo,spn,ra,kkdr,kc)
...
!            c computing the matrix elements independent of l
            ...
            do j=1,ndc+1
                if ( iphase(j)==1 ) then
                    ...
                    call calhml( nlayer(j),coef1(j),coef2(j),&
                     h3mx(-2,jtmp),h3my(-2,jtmp),h3mz(-2,jtmp),&
                     h5mx(-1,jtmp),h5my(-1,jtmp),h5mz(-1,jtmp),&
                     h4m1L(-1,jtmp),h4m1N(-1,jtmp),h4m2L(-2,jtmp),h4m2N(-2,jtmp),&
            ...
            kc = 1
            do l=0,maxlmax      ! l-loop
                tmpc(1:maxnlay+1) = dcmplx(0.d0)  ! initialize
                if ( ismall>20 ) then
                    if(llog>l) llog = l
                    exit
                endif
